...
// 获取当前认证状态
const getAuthStatus = async (): Promise<GitHub.AuthStatus> => {
  try {
...
// 处理授权回调
const handleCallback = async (url: string): Promise<GitHub.AuthResult | undefined> => {
...
// 获取当前登录用户信息
const getCurrentUser = async (token: string): Promise<GitHub.GitHubUser> => {
  try {
...
const getStarredRepositories = async (): Promise<GitHub.Repository[]> => {
  return dbService.githubRepositories.getAll()
}

// 获取用户已加星标的仓库
const syncStarredRepositories = async () => {
  try {
    const token = await getAccessToken()
    // 使用分页获取所有星标仓库
    let page = 1
    const perPage = 100
    let allRepos: GitHub.Repository[] = []
    let hasMore = true
    ...
    // 将仓库信息存储到数据库之中
    dbService.githubRepositories.sync(allRepos)
    getMainWindow()?.webContents.send('github-starred-repos-updated', allRepos)
  } catch (error: any) {
...
const logout = async (): Promise<boolean> => {
  console.log('正在登出GitHub...')
  // 清除数据库
  dbService.githubRepositories.clear()
