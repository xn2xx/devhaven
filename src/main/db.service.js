...
const dbService = {
  ...
  folders: {
    ...
    getChildren: (parentId) => {
      const stmt = db.prepare("SELECT * FROM folders WHERE parent_id = ? ORDER BY order_index, name ASC");
      return stmt.all(parentId);
    },

    // 获取根文件夹（没有父文件夹的文件夹）
    getRoots: () => {
      const stmt = db.prepare("SELECT * FROM folders WHERE parent_id IS NULL ORDER BY order_index, name ASC");
      return stmt.all();
    },
    ...
    create: (folder) => {
      ...
      if (folder.parent_id) {
        ...
        const stmt = db.prepare("SELECT MAX(order_index) as max_order FROM folders WHERE parent_id = ?");
        const result = stmt.get(folder.parent_id);
        orderIndex = (result.max_order || 0) + 1;
      } else {
        // 获取根文件夹中最大的order_index
        const stmt = db.prepare("SELECT MAX(order_index) as max_order FROM folders WHERE parent_id IS NULL");
    ...
    update: (id, data) => {
      ...
      // 检查是否改变了父文件夹
      const parentChanged = data.parent_id !== undefined &&
        (data.parent_id !== folder.parent_id ||
          (data.parent_id === null && folder.parent_id !== null));

      console.log("更新文件夹:", id,
        "原parent_id:", folder.parent_id,
        "新parent_id:", data.parent_id,
        "父级已改变:", parentChanged);
