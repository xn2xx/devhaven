...
const { loadEnv } = require("electron-vite");
...
const CLIENT_ID = env.MAIN_VITE_GITHUB_CLIENT_ID;
const CLIENT_SECRET = env.MAIN_VITE_GITHUB_CLIENT_SECRET;
const REDIRECT_URI = env.MAIN_VITE_GITHUB_REDIRECT_URI;
...
class GitHubService {
  ...
  async handleCallback(url) {
    ...
    try {
      ...
      if (state !== savedState) {
        ...
        if (this.authResolver) {
          this.authResolver({ success: false, error: "State parameter mismatch, possible CSRF attack" });
          this.authResolver = null;
        }
        return;
      }
  ...
  async getStarredRepositories() {
    try {
      ...
      while (hasMore) {
        const response = await axios.get(`https://api.github.com/user/starred?page=${page}&per_page=${perPage}`, {
          headers: {
            Authorization: `token ${token}`
          }
        });
