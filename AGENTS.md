...
- 终端 worktree 创建/打开/删除/同步（在“已打开项目”列表为父项目创建并展示 worktree 子项，支持已有/新建分支、打开仓库已存在 worktree、可选创建后打开；目标目录固定为 `~/.devhaven/worktrees/<project>/<branch>`，打开时默认继承父项目 tags/scripts；删除会执行 `git worktree remove` 并移除记录；终端打开或点击“刷新 worktree”会从 `git worktree list` 同步 `Project.worktrees` 记录）：`src/components/terminal/TerminalWorkspaceWindow.tsx`（入口/子项列表/刷新/删除）+ `src/components/terminal/WorktreeCreateDialog.tsx`（创建弹窗）；前端：`src/services/gitWorktree.ts`（`gitWorktreeAdd/gitWorktreeList/gitWorktreeRemove`）+ `src/state/useDevHaven.ts`（`syncProjectWorktrees`）↔ 后端：`src-tauri/src/git_ops.rs`（`add_worktree/list_worktrees/remove_worktree`）/`src-tauri/src/project_loader.rs`（扫描过滤 worktree 顶层目录）；Command：`src-tauri/src/lib.rs`（`git_worktree_add/git_worktree_list/git_worktree_remove`）；持久化：`projects.json` 的 `Project.worktrees`
